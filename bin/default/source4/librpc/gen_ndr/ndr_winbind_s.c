...
static NTSTATUS winbind__op_bind(struct dcesrv_call_state *dce_call, const struct dcesrv_interface *iface, uint32_t if_version)
{
...
static NTSTATUS winbind__op_ndr_pull(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, struct ndr_pull *pull, void **r)
{
	...
	*r = talloc_named(mem_ctx,
			  ndr_table_winbind.calls[opnum].struct_size,
			  "struct %s",
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		dcerpc_log_packet(dce_call->conn->packet_log_dir, 
				  &ndr_table_winbind, opnum, NDR_IN,
...
static NTSTATUS winbind__op_dispatch(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 0: {
		...
		dcesrv_winbind_information(dce_call, mem_ctx, r2);
	...
	case 1: {
		...
		r2->out.result = dcesrv_winbind_remote_control(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 2: {
		...
		r2->out.result = dcesrv_winbind_SamLogon(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
	...
	case 3: {
		struct winbind_get_idmap *r2 = (struct winbind_get_idmap *)r;
		if (DEBUGLEVEL >= 10) {
			NDR_PRINT_FUNCTION_DEBUG(winbind_get_idmap, NDR_IN, r2);
		}
		r2->out.result = dcesrv_winbind_get_idmap(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
			DEBUG(5,("function winbind_get_idmap will reply async\n"));
		}
		break;
	}
	case 4: {
		...
		r2->out.result = dcesrv_winbind_DsrUpdateReadOnlyServerDnsRecords(dce_call, mem_ctx, r2);
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
			DEBUG(5,("function winbind_DsrUpdateReadOnlyServerDnsRecords will reply async\n"));
		}
		break;
	}
...
static NTSTATUS winbind__op_reply(struct dcesrv_call_state *dce_call, TALLOC_CTX *mem_ctx, void *r)
{
	...
	case 3: {
		struct winbind_get_idmap *r2 = (struct winbind_get_idmap *)r;
		if (dce_call->state_flags & DCESRV_CALL_STATE_FLAG_ASYNC) {
			DEBUG(5,("function winbind_get_idmap replied async\n"));
		}
		if (DEBUGLEVEL >= 10 && dce_call->fault_code == 0) {
			NDR_PRINT_FUNCTION_DEBUG(winbind_get_idmap, NDR_OUT | NDR_SET_VALUES, r2);
		}
		if (dce_call->fault_code != 0) {
			DEBUG(2,("dcerpc_fault %s in winbind_get_idmap\n", dcerpc_errstr(mem_ctx, dce_call->fault_code)));
		}
		break;
	}
	case 4: {
		...
		if (dce_call->fault_code != 0) {
			DEBUG(2,("dcerpc_fault %s in winbind_DsrUpdateReadOnlyServerDnsRecords\n", dcerpc_errstr(mem_ctx, dce_call->fault_code)));
		}
		break;
	}
...
const struct dcesrv_interface dcesrv_winbind_interface = {
	.name		= "winbind",
...
static NTSTATUS winbind__op_init_server(struct dcesrv_context *dce_ctx, const struct dcesrv_endpoint_server *ep_server)
{
	...
	for (i=0;i<ndr_table_winbind.endpoints->count;i++) {
		...
		ret = dcesrv_interface_register(dce_ctx, name, &dcesrv_winbind_interface, NULL);
...
{
	if (dcesrv_winbind_interface.syntax_id.if_version == if_version &&
		GUID_equal(&dcesrv_winbind_interface.syntax_id.uuid, uuid)) {
